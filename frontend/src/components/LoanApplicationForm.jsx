...
export default function LoanApplicationForm({ onSubmit, isSubmitting }) {
  const [values, setValues] = useState({ loanAmount: '', assetValue: '', creditScore: '' });
  ...
  function handleSubmit(event) {
    ...
    if (!Number.isFinite(loanAmount) || !Number.isFinite(assetValue) || !Number.isFinite(creditScore) || loanAmount <= 0 || assetValue <= 0) {
  ...
  return (
    <section className="panel form-panel">
      <div>
        ...
        <p className="muted">Enter the applicant’s details and let the lending API make the decision.</p>
      </div>
      <form onSubmit={handleSubmit} noValidate>
        <label htmlFor="loanAmount">Loan amount (£)</label>
        <input id="loanAmount" name="loanAmount" type="number" min="0.01" step="0.01" value={values.loanAmount} onChange={handleChange} placeholder="500000" required />

        <label htmlFor="assetValue">Asset value (£)</label>
        <input id="assetValue" name="assetValue" type="number" min="0.01" step="0.01" value={values.assetValue} onChange={handleChange} placeholder="800000" required />

        <label htmlFor="creditScore">Credit score (1–999)</label>
        <input id="creditScore" name="creditScore" type="number" min="1" max="999" step="1" value={values.creditScore} onChange={handleChange} placeholder="850" required />

        {validationError && <p className="form-error" role="alert">{validationError}</p>}
        <button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Checking…' : 'Submit application'}</button>
      </form>
    </section>
  );
}
